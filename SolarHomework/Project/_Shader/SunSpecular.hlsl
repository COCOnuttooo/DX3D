...
float4 PS(VertexOutput input) : SV_TARGET
{
    ...
    float3 light = normalize(float3(lightDirection2.x,lightDirection2.y,lightDirection2.z));
    
    float diffuseIntensity;
    if (lightDirection2.w == 0)
        diffuseIntensity = 1;
    else
        diffuseIntensity = saturate(dot(input.normal, -light));
